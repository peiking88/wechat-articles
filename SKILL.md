...
被拦形态共三种，处置不同：① 约 17KB 空壳页（无 `js_content`、title 空，参数链接常见）→ 换短链重试；② "环境异常"验证页 → 风控，走 Step 4 冷却；③ 约 2KB "未知错误，请稍后再试"页 → 瞬时拒绝非风控，补全 `Accept-Language` 等浏览器头隔 3 秒重试即过，无需冷却（2026-09-08 实测）。**形态③若补头后重试 2-3 次仍持续、且短链与参数链接同拒 → 已不是瞬时拒绝，是本机 IP 全域被拦，冷却也救不回来，直接跳 Step 4 的服务端兜底链**（2026-09-17 实测 7 连拒、冷却 25 分钟无效）。
...
**本机全域被拦时的服务端兜底链**（2026-09-17 实测全通，判定条件见 Step 1 形态③加粗句；此状态下拿不到文章原始 HTML → 无 `__biz`/`album_id` → 合集页不可用，链路重建如下）：

1. **webReader 可正常抓文章页**（服务端出口独立），但输出无原始 HTML，只能取 og:title 与正文——正文含作者手写的"上一篇回顾/下一篇预告"，本身就是滚雪球线索
2. **搜狗微信搜索本机可直连**（`https://weixin.sogou.com/weixin?type=2&query=<关键词>`，7s 间隔实测约 20 次不触发验证码）；结果在 `<div class="txt-box">` 块内：h3 = 微信 title 标签（主题词+篇号，比 og:title 信息量大），`txt-info` 摘要**常含"下一篇预告：《标题》"全文**——翻页（page=2/3）+ 主题词变体可枚举大部分篇目，比逐篇抓正文便宜一个数量级
3. **搜狗 `/link?url=` 跳转链接与会话绑定**，几分钟即失效，跨会话复用返回 302 到首页；必须同一 curl 会话原子完成：搜索（`-c` 存 cookie）→ 从**本次结果页**提取 /link → 带 cookie + 该搜索页 Referer 请求 → 跳转页 JS 按 `url += '...'` 分段拼出真实参数 URL（`src=11` 带 signature，有时效，立即交给 webReader 用）
4. 拿参数 URL 逐篇 webReader 抓正文，用尾部"下一篇预告"滚雪球，补齐搜狗未索引的篇目
